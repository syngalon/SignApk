...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    private Object mAudioPatchLock = new Object();  // zhanghang add MTK patch for fm no sound, 2017.05.02
    ...
    private void stopAudioTrack() {
        ...
        try {
                // [ALPS03366129] zhanghang modify for no sound when shooting video in lock screen, 2017.06.28, begin
    ...
    private boolean powerUp(float frequency) {
        ...
        // zhanghang add MTK patch for fm no sound, 2017.05.02, begin
        ...
        // zhanghang add MTK patch for fm no sound, 2017.05.02, end
    ...
    private synchronized void createAudioPatch() {
        ...
        Log.d(TAG, "createAudioPatch, mAudioPatch created = " + mAudioPatch);   // zhanghang add MTK patch for fm no sound, 2017.05.02
    }
    ...
    private class FmOnAudioPortUpdateListener implements OnAudioPortUpdateListener {
        ...
        public void onAudioPatchListUpdate(AudioPatch[] patchList) {
            ...
            }  // zhanghang add MTK patch for fm no sound, 2017.05.02
        }
        ...
        for (AudioPatch patch : patches) {
            ...
            if (sourcePort instanceof AudioMixPort && sinkPort instanceof AudioDevicePort) {
                ...
                if (type == AudioSystem.DEVICE_OUT_BLUETOOTH_A2DP ||
                    ...
                    type == AudioSystem.DEVICE_OUT_AUX_DIGITAL ||
                    type == AudioSystem.DEVICE_OUT_USB_ACCESSORY ||
                    type == AudioSystem.DEVICE_OUT_USB_DEVICE ||
    ...
    private void showPlayingNotification() {
        ...
        Log.d(TAG, "showPlayingNotification PS:" + stationName + ", RT:" + radioText + ", mCurrentStation:" + mCurrentStation);
        /*Bitmap largeIcon = FmUtils.createNotificationLargeIcon(mContext,
                FmUtils.formatStation(mCurrentStation));
        views.setImageViewBitmap(R.id.iv_cover, largeIcon);*/
        views.setTextViewText(R.id.txt_radio, FmUtils.formatStation(mCurrentStation));
    ...
    public boolean requestAudioFocus() {
        ...
        Log.d(TAG, "requestAudioFocus, mIsAudioFocusHeld" + mIsAudioFocusHeld);  // zhanghang add MTK patch for fm no sound, 2017.05.02
    ...
    private void notifyActivityStateChanged(Bundle bundle) {
        Log.i(TAG, "notifyActivityStateChanged: " + mRecords.size());
        if (!mRecords.isEmpty()) {
            synchronized (mRecords) {
    ...
    private boolean firstPlaying(float frequency) {
        ...
        float seekStation = 0;
	String versionname=SystemProperties.get("ro.build.display.id");
	Log.d("FmService", "versionname: " + versionname);
	int index=versionname.lastIndexOf("_");
	String str=versionname.substring(index+1);
	Log.d("FmService", "str: " + str);
	if(str.equals("RU")){  seekStation=frequency;
	}else {  	seekStation = FmNative.seek(frequency, false);}
    ...
    private boolean isMusicOutputId(int sourcePortId) {
        ...
        Log.d(TAG, "getMusicOutputId = " + mOutputPortId + "sourcePortId = " + sourcePortId);
        if (mOutputPortId == 0) {
